    ...
    try:
        ...
        if (pred_dir / f"{model_id}_valid_raw.csv").exists():
            ...
            for target_idx, target in enumerate(TARGETS):
                ...
                x_train = train.loc[mask, feature_cols].replace([np.inf, -np.inf], np.nan)
                y_train = y.loc[mask]
                x_fit, y_fit = sample_xy(x_train, y_train, min(args.sample_rows, 7000), args.random_state + target_idx)
                imputer = SimpleImputer(strategy="median")
                x_fit_np = imputer.fit_transform(x_fit)
                x_valid_np = imputer.transform(x_valid_raw)
                estimator = NGBRegressor(Dist=Normal, n_estimators=180, learning_rate=0.04, random_state=args.random_state, verbose=False)
                t0 = time.perf_counter()
            ...
            rows.append({"model_id": model_id, "model_family": "NGBoost", "status": "ok", "notes": "NGBoost Normal distribution first-pass sampled", "train_seconds": train_seconds, "predict_seconds": predict_seconds})
    except Exception as exc:
...
def residual_stats(model_id: str, y_true: pd.DataFrame, pred: pd.DataFrame, baseline_pred: pd.DataFrame) -> list[dict]:
    rows = []
    for target in TARGETS:
        cap = CAPACITY[target]
...
def run_evaluate_phase(args: argparse.Namespace) -> None:
    ...
    for model_id, pred in pred_bank.items():
        residual_rows.extend(residual_stats(model_id, y_true, pred, baseline_pred))
    ...
    if not residual_df.empty:
        diversity = (
            residual_df[residual_df["model_id"] != "lgbm_l1_baseline"]
        ...
        for other in diversity:
            if other in pred_bank:
                ensemble_rows.append(evaluate_ensemble(f"diverse_lgbm_{other}_0.90", ["lgbm_l1_baseline", other], [0.9, 0.1], pred_bank, y_true, timestamps))
    ...
    accepted = bool(
        candidate["alpha_score"] >= ACCEPT_SCORE
        and candidate.get("alpha_ficr", 0.0) >= BASELINE_ALPHA
    )
    # Strict acceptance below: baseline alpha is not FiCR, so recompute by comparing against baseline row.
    baseline_row = ok[ok["model_id"] == "lgbm_l1_baseline"].iloc[0].to_dict()
    accepted = bool(
        candidate["alpha_score"] >= ACCEPT_SCORE
        and candidate.get("alpha_ficr", 0.0) >= baseline_row.get("alpha_ficr", 0.0)
        and (math.isnan(float(candidate.get("worst_month_score", np.nan))) or candidate.get("worst_month_score", 0.0) >= baseline_row.get("worst_month_score", 0.0) - 0.002)
    )
...
def write_conclusion(results_dir: Path, model_summary: pd.DataFrame, residual_df: pd.DataFrame, ensemble_df: pd.DataFrame, best_params: dict) -> None:
    ...
    if not residual_df.empty:
        low_corr = residual_df[residual_df["model_id"] != "lgbm_l1_baseline"].groupby("model_id", as_index=False).agg(mean_pearson=("pearson", "mean"), better_abs_error_rate=("model_better_abs_error_rate", "mean")).sort_values("mean_pearson").head(12)
    lines = [
        ...
        f"- baseline to beat: W4 + LightGBM L1 + global alpha {BASELINE_ALPHA}",
        f"- local baseline score: {BASELINE_LOCAL_SCORE}",
        f"- Public LB baseline: {BASELINE_PUBLIC_LB}",
        f"- acceptance score threshold: {ACCEPT_SCORE}",
        "",
        "## Execution coverage",
        "",
        ...
        "## Top single models",
        "",
        top_models[["model_id", "model_family", "raw_score", "best_alpha", "alpha_score", "delta_vs_baseline", "alpha_ficr", "worst_month_score", "high_generation_score", "months_better_than_lgbm", "groups_better_than_lgbm", "status", "notes"]].to_markdown(index=False),
        "",
        "## Top ensembles",
        "",
